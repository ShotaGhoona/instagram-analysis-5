...
│   └── analytics_service.py    # 分析データ処理ロジック
│
├── api/                         # FastAPI エンドポイント
│   ├── __init__.py
│   ├── auth.py                 # 認証API（旧auth/simple_auth.py）
...
#### 3. **services/** - ビジネスロジック層（ユースケース）
- 複雑なビジネスロジックの実装
- 複数のRepositoryの組み合わせ
- トランザクション管理

#### 4. **api/** - インターフェース層（コントローラー）
- HTTPリクエスト/レスポンスの処理
- バリデーション
- Serviceの呼び出しのみ

#### 5. **external/** - 外部統合層
- Instagram API等の外部サービス連携
- 外部APIクライアントの実装
